...
Order.create = function(meta,storeAddr,submarketAddr,feePercentage,disputeSeconds,affiliateAddr){
	...
	keyGroup.promise.then(function(keyGroup){
		...
		keyGroup.encrypt(meta).then(function(pgpMessage){
			...
			txMonitor.propose('Create a New Order',Order.contractFactory,[meta,storeAddr,submarketAddr,feePercentage,disputeSeconds,affiliateAddr,contracts.OrderBook.address,{data:order.code}]).then(function(receipt){
				console.log(receipt)
